...
export const authConfig = {
  ...
  cookies: {
    callbackUrl: {
      ...
      options: {
        domain: process.env.ASTER__COOKIES__DOMAIN || undefined,
      },
    },
    csrfToken: {
  ...
  debug: (process.env.ASTER__DEBUG || "true") === "true",
  pages: {
    error: "/auth/error",
    newUser: "/auth",
    signIn: "/auth/login",
  ...
  secret: splitSecrets(process.env.ASTER__SECRETS__AUTH || "secret"),
  session: {
